...
|- **Task Manager Block** — `Block-TaskManager` uses three simultaneous mechanisms: (1) IFEO Debugger redirect pointing to a non-existent executable so `taskmgr.exe` silently fails on launch; (2) `DisableTaskMgr` registry policy set to `1`; (3) hardened IFEO registry key ACLs (SYSTEM FullControl, Admins ReadKey-only) to prevent tampering. Automatically enabled when God Mode is active and restored on disable.
...
13. **Task Manager Block**: Three simultaneous mechanisms prevent Task Manager from launching while God Mode is active: (1) IFEO Debugger redirect to a non-existent executable so `taskmgr.exe` silently fails on launch; (2) `DisableTaskMgr` registry policy set to `1`; (3) hardened IFEO registry key ACLs (SYSTEM FullControl, Admins ReadKey-only) to prevent tampering. Automatically restored when God Mode is disabled.
