...
| [Smart IFEO Compatibility Layer](#smart-ifeo-compatibility-layer) | Detector A/B auto-exclude + hardening (AppX/Store-stub drop = the notepad fix, reason field, force-system seam, mtime + token invalidation, cache, mutex fallback, per-app REASON column) |
