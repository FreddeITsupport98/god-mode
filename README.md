...
| [Smart IFEO Compatibility Layer](#smart-ifeo-compatibility-layer) | Detector A/B auto-exclude + hardening (AppX/Store-stub drop = the notepad fix, reason field, force-system seam, mtime + token invalidation, cache, mutex fallback, per-app REASON column; 2026-07-19 20:58: notepad detection-miss fix = curated Win11-stub fallback + WindowsApps FS scan, same-dir copy fallback, 5-min store reconcile, $reasonLegend hashtable) |
...
### AppX/Store-redirector stub drop + 3 suggestions (2026-07-19)

Six additive improvements that fix Win11 notepad (and every AppX/WinUI Store-redirector stub: mspaint, calc, photos, snippingtool, ...) NOT starting via gmproxy, plus the 3 hardening suggestions. All backward-compatible + fail-open; no hardcoded app names.

- **AppX/Store-stub drop = the notepad fix (Detector A strategy flip).** Win11 `C:\Windows\notepad.exe` is a ~300KB Store-app redirector stub: the App Execution Alias reparse point at `%LOCALAPPDATA%\Microsoft\WindowsApps\notepad.exe` (tag `0x8000001B`) + App Paths redirect the launch to the WinUI Store Notepad. gmproxy's IFEO-bypass makes a **renamed copy** `gmproxy_<pid>_notepad.exe` in Temp (the same-dir hardlink to `C:\Windows\` fails because gmproxy runs as the admin user for filesystem ops), which breaks the stub: the alias redirect does not fire under the renamed name AND the `.mui` resource sidecar can't be found -> the stub exits 0 with no window as BOTH SYSTEM and user, so Detector B's auto-exclude (re-launch the same renamed copy) never helps. `Get-GmSystemCompatExclusions` already DETECTED AppX stubs but the strategy kept them IFEO-hooked. Now `Install-IfeoElevation` + `Get-IfeoElevationCandidates` DROP `$CompatExclusions.AppX` from the seed + auto-candidates (like browsers), persist the FULL AppX set to the auto-exclude store with reason 'A' (gmproxy + gmhook + monitor skip them from launch #1), and REMOVE any prior IFEO hook for them on a re-enable (so a VM that hooked notepad cleans up). `Add-IfeoElevationForApp` (instant watcher) skips App Execution Alias stubs (reparse-point check). `Add-GmAutoExcludeEntries` gains a `-Reason` param (default 'P' browser, 'A' AppX). AppX apps fundamentally cannot run as SYSTEM (package activation needs user identity), so native user launch via the alias is the only way they start. **VM deploy: re-run Enable God Mode so Detector A removes the prior notepad IFEO hook.**
- **gmproxy.c alias-stub guard (belt-and-suspenders).** `GmProxyIsAppExecutionAliasStub(base)` checks the WindowsApps reparse point; before recording a CLEAN-GUI refusal, if the target is an alias stub the guard SKIPS the record + logs the real cause (`[GM-PROXY] AUTO-EXCLUDE: skip record -- <base> is an App Execution Alias stub ...`). CRASH is still recorded. Fail-open. Catches any stub that slips past Detector A (alias created after install).
- **Suggestion 1 -- `Reset-GmProxyAutoExcludeStore` clears the `Test-GmAutoExcluded` 15s cache** so a monitor scan right after menu [18] doesn't see stale exclusions.
- **Suggestion 2 -- per-app REASON column in `Export-GodModeLogs`** (option [11]): the per-app launch report now shows the stored reason per app (C/G/P/A/?) + an `A=ALIAS-STUB` legend, so the dump is self-explanatory without reading the raw store.
- **Suggestion 3 -- gmhook cache invalidates on a host-process-token change** (`cacheLoadTokenSid` + `EqualSid`): if the monitor elevates the host to SYSTEM in-place (or a token swap mid-session), the store reloads immediately. Completes the always-fresh guarantee alongside mtime + TTL. Fail-open.
- **Store reason `A=ALIAS-STUB`** added to the 5th-field legend (alongside C/G/P/?).

### Notepad detection-miss fix + 3 hardening suggestions (2026-07-19 20:58 UTC)

A fresh VM dump showed notepad STILL did not start even after the 2026-07-19 20:02 AppX-drop edit (build `Jul 19 2026 13:27:09`, `droppedAppx=1`/`persistedAppx=48`, but `notepad.exe` was NOT among the 48 persisted `'A'` entries -- only `notepad++.exe` was; `droppedAppx=1` = notepad++). Root cause: `Get-GmSystemCompatExclusions` FAILED to classify `notepad.exe` as AppX on this VM -- the WindowsApps reparse scan found no admin alias AND `Get-AppxPackage` (per-user + `-AllUsers`) did not surface `Microsoft.WindowsNotepad`'s manifest Executable -- so notepad was NOT AppX-dropped, NOT persisted `'A'`, and the IFEO Debugger set succeeded (gmproxy invoked via IFEO) whose renamed Temp copy broke the stub. The gmproxy alias guard `GmProxyIsAppExecutionAliasStub` did not fire (no admin WindowsApps alias). Five additive fixes:

- **N1 -- Notepad fix: curated Win11-stub fallback + WindowsApps FS scan in `Get-GmSystemCompatExclusions`.** Added AppX source (4): a direct filesystem scan of `C:\Program Files\WindowsApps\*\AppXManifest.xml` parsing the `Executable=` manifest regex (catches packages `Get-AppxPackage` misses on accounts where the package isn't registered). Added AppX source (5): a small curated Win11-stub list (`notepad.exe`, `mspaint.exe`, `calc.exe`, `snippingtool.exe`) -- the FIXED, KNOWN set Microsoft replaced with Store versions -- each `Test-Path`-validated against `C:\Windows\<name>` / `C:\Windows\System32\<name>` so a name absent on this VM is NOT classified, only added when not already detected dynamically. Detector A then drops the stubs from the IFEO seed + the `appxHookRemoved` cleanup removes any prior gmproxy Debugger + persists them `'A'` excluded=1 -> gmhook's `GmHookIsAutoExcluded` returns TRUE -> gmhook falls through to the real `CreateProcessW` -> notepad launches NATIVELY (no gmproxy, no rename) -> the stub's redirect opens the Store app as the user. THIS is the actual notepad fix. The dynamic reparse + AppxPackage sources stay primary; the curated list + WindowsApps FS scan are safety nets for VMs where all dynamic detection misses the classic stubs.
- **N2 -- Suggestion 3: gmproxy same-directory copy fallback (SYSTEM-impersonation retry).** When `CreateHardLinkW` fails, gmproxy now tries a same-dir `CopyFileW` under its current token, then under the stolen SYSTEM token (`DuplicateTokenEx` -> `TokenImpersonation` + `ImpersonateLoggedOnUser` + `RevertToSelf`) so ACL-protected dirs (`C:\Windows`/`System32`) the admin can't write stay in-context, then falls back to Temp. Keeps the canonical-directory context for path-relative non-AppX targets. Fail-open.
- **N3 -- Suggestion 1: `Invoke-GmAutoExcludeReconcile` + monitor 5-min cadence.** New function prunes orphaned `'A'` store entries (stub + alias gone = Store app uninstalled) under the `Global\GmProxyAutoExcludeMutex`, atomic temp+rename, never touches `C`/`G`/`P`. Called from `Start-Monitoring` every 5 min via a new `$lastReconcile` timer. Fail-open.
- **N4 -- Suggestion 2: dynamic `$reasonLegend` hashtable in `Export-GodModeLogs`.** A single `[ordered]@{ C=...; G=...; P=...; A=...; ?=... }` hashtable now drives BOTH the store reason legend + the per-app REASON legend (`$reasonParts` + `$reasonParts2`) so a future reason code updates both in lockstep (no drift). Additive, display-only.
- **N5 -- Belt-and-suspenders: gmproxy `GmProxyAutoExcludeRecord` preserves reason `'A'`.** The stored reason is only overwritten when it is not `'A'` (an install-time AppX drop is more specific than a runtime `G`/`C`; a stub that slips past Detector A keeps its `'A'`). One-line guard; fail-open.
- **Regression:** `Test-GmProxySession.ps1` updated 2 legend asserts + new section 27 (+21); `Test-IfeoElevation.ps1` new section 12c (+11); `test-gmproxy-session.sh` + `test-gmproxy-refuse.sh` +greps (same-dir copy, impersonation, `'A'`-preserve). MinGW rebuild required for the gmproxy.c changes (PE32+). **VM deploy: copy rebuilt `gmproxy.exe` + `gmhook.dll` to `C:\ProgramData\GodMode`, re-run Enable God Mode so Detector A drops notepad from IFEO + the cleanup removes the prior notepad Debugger.** NOT pushed.

### mmc.exe CLEAN-GUI pre-seed (2026-07-21)

`mmc.exe` (hosts every .msc snap-in: services, eventvwr, compmgmt, gpedit, secpol, lusrmgr, ...) silently refuses SYSTEM — it launches as SYSTEM, exits 0, renders no window (the MMC host relies on per-user profile/COM resources). Detector B learned this after 2 SYSTEM attempts and auto-excluded it, but the 2 one-time flash-and-disappear attempts looked like "admin tools crashing." `Install-IfeoElevation` now pre-seeds `mmc.exe` in the auto-exclude store with reason `'G'` (CLEAN-GUI) at install time via `Add-GmAutoExcludeEntries`, so gmproxy launches it as the current user from the FIRST launch with no visible flash. The IFEO hook is retained so menu [18] RESET AUTO-EXCLUDE STORE retries SYSTEM (the next ToggleOn re-seeds it, same as browser/AppX pre-seeds). `perfmon.exe` + `resmon.exe` are NOT pre-seeded (thin launchers that delegate to mmc.exe; once mmc.exe is pre-excluded, the .msc GUI runs as the user from launch #1). Merge-safe + fail-open.
...
- **2026-07-21 18:43 UTC** — Pre-seeded `mmc.exe` in the Detector B auto-exclude store with reason `'G'` (CLEAN-GUI) so admin tools launch as the current user from the FIRST launch, skipping the 2 one-time SYSTEM flash-and-disappear attempts that looked like "admin tools crashing." `mmc.exe` (hosts every .msc snap-in: services/eventvwr/compmgmt/gpedit/secpol/lusrmgr/...) silently refuses SYSTEM (exits 0, no window — the MMC host relies on per-user profile/COM resources). `Install-IfeoElevation` now calls `Add-GmAutoExcludeEntries -BaseNames @('mmc.exe') -Reason 'G'` at install time so gmproxy launches it as the current user from launch #1 (IFEO hook retained; menu [18] RESET retries SYSTEM; next ToggleOn re-seeds). `perfmon.exe` + `resmon.exe` are NOT pre-seeded (thin launchers that delegate to mmc.exe; once mmc.exe is pre-excluded, the .msc GUI runs as the user from launch #1). Seed comment corrected from "known-good as SYSTEM" to "silently refuses SYSTEM." Summary log + debug EXIT report `preseededCleanGui`. Merge-safe + fail-open. Regression: `Test-GmProxySession.ps1` +6 section 37; `Test-IfeoElevation.ps1` +5 section 12b-preseed. PowerShell-only (no C rebuild). NOT pushed.

- **2026-07-19 20:58 UTC** — Fixed the residual "notepad STILL does not start" case the 20:02 edit missed + implemented the 3 approved hardening suggestions. Root cause: `Get-GmSystemCompatExclusions` did NOT classify `notepad.exe` as AppX on the VM (the 48 persisted `'A'` entries did not include notepad; only `notepad++.exe` was; `droppedAppx=1` = notepad++) -- the WindowsApps reparse scan found no admin alias + `Get-AppxPackage` did not surface `Microsoft.WindowsNotepad`, so notepad was not AppX-dropped, not persisted `'A'`, and the IFEO Debugger set succeeded (gmproxy's renamed Temp copy broke the stub). (1) **Notepad fix**: `Get-GmSystemCompatExclusions` gains (4) a direct `C:\Program Files\WindowsApps\*\AppXManifest.xml` manifest scan + (5) a curated Win11-stub fallback (`notepad`/`mspaint`/`calc`/`snippingtool`, `Test-Path`-validated) so the classic stubs are classified AppX even when `Get-AppxPackage` misses them -> Detector A drops them from IFEO + persists `'A'` + gmhook's store consult skips -> native launch -> Store app opens. (2) **Suggestion 3**: gmproxy same-dir copy fallback (current token, then SYSTEM impersonation via `DuplicateTokenEx`/`ImpersonateLoggedOnUser`/`RevertToSelf`) before the Temp fallback. (3) **Suggestion 1**: `Invoke-GmAutoExcludeReconcile` prunes orphaned `'A'` entries on a 5-min monitor cadence (never touches C/G/P). (4) **Suggestion 2**: a single `$reasonLegend` hashtable drives both the store + per-app REASON legends (no drift). (5) gmproxy `GmProxyAutoExcludeRecord` preserves reason `'A'` (belt-and-suspenders). Regression: `Test-GmProxySession.ps1` +21 section 27 + 2 legend-assert updates; `Test-IfeoElevation.ps1` +11 section 12c; `test-gmproxy-session.sh` + `test-gmproxy-refuse.sh` +greps. MinGW rebuild required (PE32+). **VM deploy: copy rebuilt gmproxy.exe + gmhook.dll to `C:\ProgramData\GodMode`, re-run Enable God Mode.** NOT pushed.

- **2026-07-19 20:02 UTC** — Fixed Win11 notepad (and AppX/WinUI Store-redirector stubs: mspaint/calc/photos/snippingtool/...) NOT starting via gmproxy, plus the 3 hardening suggestions. Root cause: `C:\Windows\notepad.exe` is a Store-app redirector stub (App Execution Alias reparse point → WinUI Store Notepad); gmproxy's IFEO-bypass RENAMED copy (`gmproxy_<pid>_notepad.exe` in Temp) breaks the alias redirect + the `.mui` resource lookup → the stub exits 0 with no window as BOTH SYSTEM and user, so Detector B's auto-exclude (re-launch the same renamed copy) never helps. (1) **Detector A strategy flip**: `Install-IfeoElevation` + `Get-IfeoElevationCandidates` now DROP `$CompatExclusions.AppX` from the seed + auto-candidates (like browsers), persist the full AppX set to the auto-exclude store with reason 'A' (gmproxy + gmhook + monitor skip them from launch #1), and REMOVE any prior IFEO hook for them on a re-enable; `Add-IfeoElevationForApp` skips App Execution Alias stubs (reparse-point check); `Add-GmAutoExcludeEntries` gains a `-Reason` param (default 'P', 'A' for AppX). AppX apps cannot run as SYSTEM (package activation needs user identity) → native user launch via the alias is the only way they start. (2) **gmproxy.c `GmProxyIsAppExecutionAliasStub` guard** skips recording a CLEAN-GUI refusal for a base with a Store alias + logs the real cause (belt-and-suspenders for a stub that slips past Detector A; CRASH still recorded; fail-open). (3) **Suggestion 1**: `Reset-GmProxyAutoExcludeStore` clears the `Test-GmAutoExcluded` 15s cache. (4) **Suggestion 2**: `Export-GodModeLogs` per-app report gains a REASON column (C/G/P/A/?) + `A=ALIAS-STUB` legend. (5) **Suggestion 3**: gmhook `GmHookIsAutoExcluded` invalidates on a host-process-token change (`cacheLoadTokenSid` + `EqualSid`; fail-open). Regression: `Test-IfeoElevation.ps1` flipped 3 + new section 12b (+6); `Test-GmProxySession.ps1` flipped 4 + new section 26 (+15); `test-gmproxy-session.sh` + `test-gmproxy-refuse.sh` +greps (alias guard, gmhook token). MinGW rebuild required for the gmproxy.c + gmhook.c changes (PE32+). **VM deploy: copy rebuilt gmproxy.exe + gmhook.dll to `C:\ProgramData\GodMode`, re-run Enable God Mode so Detector A removes the prior notepad IFEO hook.** NOT pushed.
