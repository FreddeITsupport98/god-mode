...
Add-Assertion "God-Mode-Windows.ps1: CreateProcessAsSystem ALWAYS relocates the token to the active session before launch (unconditional SetTokenInformation, no query-then-skip)" ($gm -match 'int sid = \(int\)activeSession;\s*\n\s*if \(!SetTokenInformation\(hPrimaryToken, TokenSessionId, ref sid, 4\)\)') "CreateProcessAsSystem does not unconditionally pin the token to the active session before launch -- a Session-0 caller (the monitor) would birth an invisible Session-0 child (the VM dump's 5 invisible SYSTEM shells)"
...
Add-Assertion "PS7 WMI: queue + watcher passed to the ThreadJob via -ArgumentList (runscope-safe)" ($gm -match 'function\s+Register-ProcessCreationWatcher[\s\S]{0,12000}?\-ArgumentList \$watcher, \$queue, \$state, \$query') "Register-ProcessCreationWatcher does not pass the watcher+queue+state+query to the ThreadJob via -ArgumentList -- the ThreadJob could not reach them reliably (and the RPC-self-heal catch needs the query to re-create the watcher)"
...
Add-Assertion "Heartbeat B: heartbeat auto-re-registers a dead watcher (Register-ProcessCreationWatcher)" ($gm -match 'if \(\$script:ProcessCreationWatcherActive -and \(-not \$jobAlive -or \$forceReRegister\)[\s\S]{0,1500}?Register-ProcessCreationWatcher') "heartbeat does not re-register a dead/zombie watcher -- the fast ~1s elevation path would stay down (a zombie = State=Running but Beats frozen, e.g. after an RPC server failure)"
...
Add-Assertion "Shell C: interactive shells routed to Invoke-BornAsSystemShellVisible BEFORE generic Phase 1 (verify-gated kill, not unconditional on a generic fallback)" ($gm -match 'Invoke-BornAsSystemShellVisible -ShellPath \$Path -OldAdminPid \$ProcessId[\s\S]{0,600}?# --- Phase 1: CreateProcessAsSystem') "interactive shells are not routed to Invoke-BornAsSystemShellVisible before the generic Phase 1 -- a shell would hit the generic fallback that births an invisible Session-0 shell + unconditionally kills the visible admin shell ('it just kills my shell')"
Add-Assertion "Shell C: Phase 2 is NON-SHELLS only (interactive shells return before the service kill path)" ($gm.Contains('# --- Phase 2: Service-based elevation (last resort, NON-SHELLS only) ---') -and $gm -match 'if \(\$isInteractiveShell\) \{[\s\S]{0,400}?leaving as admin') "Phase 2 still targets interactive shells -- a service births an invisible Session-0 shell + the purge would take the visible admin shell ('it just kills my shell')"
...
Add-Assertion "InstantShell: IsInteractiveShell branch signals the shell PID after birth (gated on dwProcessId != 0, on SYSTEM-birth success OR the user-birth fallback)" ($gmhook -match 'if \(IsInteractiveShell\(baseName\)\) \{[\s\S]{0,3000}?lpProcessInformation && lpProcessInformation->dwProcessId != 0\) \{[\s\S]{0,40}?SignalShellBirth\(lpProcessInformation->dwProcessId\)') "the IsInteractiveShell branch does not signal the shell PID after birth -- the monitor is not notified to verify/skip the SYSTEM shell"
...
Add-Assertion "InstantShell: handler delegates Phase 0 failure to Invoke-BornAsSystemShellVisible (visible verify + conditional kill, no inline unconditional kill)" ($ishBody -match 'Invoke-BornAsSystemShellVisible -SystemPid \$systemPid -ShellPath \$shellPath -OldAdminPid \$ProcessId' -and -not ($ishBody -match 'Stop-Process -Id \$ProcessId -Force')) "handler does not delegate to Invoke-BornAsSystemShellVisible (or still kills inline) -- an unconditional kill on Phase 0 failure would birth an invisible Session-0 shell + kill the visible admin shell ('it just kills my shell')"
}
...
# (2026-07-23). The user reported "RPC server failure when explorer events".
# Root cause: ManagementEventWatcher.WaitForNextEvent() throws
# RPC_S_SERVER_UNAVAILABLE (0x800706BA) when explorer restarts / WMI service
# hiccups. The old catch block swallowed it + retried on the SAME dead watcher
# -- which keeps throwing RPC-unavailable forever, so the ThreadJob spun in a
# 500ms loop with Beats frozen (a ZOMBIE: State=Running but delivering nothing).
# The heartbeat only re-registered on State!=Running, so it never caught the
# zombie. Fix: (1) the ThreadJob catch disposes the dead watcher + creates a
# FRESH one (self-heal), surfaces the error via $st, and breaks after 5
# consecutive re-create failures; (2) the heartbeat drains + logs the $st error
# AND detects a zombie (Beats frozen >=2 intervals while Running) -> force
# re-register. Additive; no behavior change to the happy path.
Add-Assertion "WmiRpc: ThreadJob catch re-creates the watcher (New-Object ManagementEventWatcher in the catch)" ($gm -match 'catch \{[\s\S]{0,1200}?\$watcher = New-Object System\.Management\.ManagementEventWatcher\(\$wmiQuery\)') "the WMI watcher catch block does not re-create the watcher -- a dead RPC subscription keeps throwing forever (the zombie the user hit)"
Add-Assertion "WmiRpc: ThreadJob passes the WMI query as a param (wmiQuery)" ($gm -match 'param\(\$watcher, \$q, \$st, \$wmiQuery\)') "the ThreadJob does not receive the WMI query -- the catch cannot re-create the watcher (New-Object would reference an out-of-scope $query)"
Add-Assertion "WmiRpc: ThreadJob catch disposes the dead watcher before re-creating" ($gm -match 'try \{ \$watcher\.Stop\(\) \} catch[\s\S]{0,80}?try \{ \$watcher\.Dispose\(\) \}') "the catch does not dispose the dead watcher -- the old WMI subscription handle leaks + the new one may collide"
Add-Assertion "WmiRpc: ThreadJob surfaces the error via shared state (WmiErrorPending + LastWmiError)" ($gm.Contains('$st.WmiErrorPending = $true') -and $gm.Contains('$st.LastWmiError = $errMsg')) "the ThreadJob does not surface the RPC error -- the user cannot see why the watcher stopped delivering"
Add-Assertion "WmiRpc: ThreadJob breaks after 5 consecutive re-create failures (WmiGaveUp)" ($gm.Contains('$consecutiveFail -ge 5') -and $gm.Contains('$st.WmiGaveUp = $true')) "the ThreadJob never gives up -- a persistently dead WMI service spins the catch loop forever instead of letting the heartbeat re-register"
Add-Assertion "WmiRpc: ThreadJob resets the failure counter on a successful event" ($gm.Contains('$consecutiveFail = 0   # event delivered -> subscription is healthy')) "the ThreadJob does not reset the failure counter on success -- a single blip after a long healthy run would hit the 5-cap + abort unnecessarily"
Add-Assertion "WmiRpc: heartbeat drains + logs the pending WMI error (WmiErrorPending)" ($gm.Contains('$script:ProcessCreationWatcherState.WmiErrorPending') -and $gm -match 'WMI watcher: RPC/event error recovered') "the heartbeat does not surface the pending WMI error -- the 'RPC server failure' stays invisible in the dump"
Add-Assertion "WmiRpc: heartbeat logs the gave-up state (WmiGaveUp)" ($gm.Contains('$script:ProcessCreationWatcherState.WmiGaveUp') -and $gm -match 'WMI watcher gave up self-healing') "the heartbeat does not surface the gave-up state -- a permanently dead WMI subscription is silently ignored"
Add-Assertion "WmiRpc: heartbeat zombie-detection counter declared (consecutiveZeroDelta)" ($gm.Contains('$consecutiveZeroDelta = 0')) "the zombie-detection counter is missing -- the heartbeat cannot detect a Running-but-frozen watcher"
Add-Assertion "WmiRpc: heartbeat forces re-register on >=2 zero-delta intervals while Running" ($gm.Contains('$consecutiveZeroDelta -ge 2') -and $gm.Contains('$forceReRegister = $true')) "the heartbeat does not force re-register a zombie -- a State=Running watcher with frozen Beats stays dead until the monitor restarts"
Add-Assertion "WmiRpc: heartbeat logs the zombie force-re-register (zombie detected)" ($gm -match 'WMI watcher zombie detected .*State=Running but no events') "the heartbeat does not log the zombie force-re-register -- the recovery is invisible"
Add-Assertion "WmiRpc: heartbeat resets the zero-delta streak when Beats increment" ($gm.Contains('$consecutiveZeroDelta = 0') -and $gm.Contains('if ($beatsDelta -le 0)')) "the heartbeat does not reset the streak on delivery -- a healthy watcher that pauses briefly would false-trigger a force re-register"

# Part F -- Win11 26100 visible born-as-SYSTEM shell fallback (2026-07-23). The
# VM dump proved Phase 0 (in-place swap) is dead on build 26100 (STATUS_NOT_
# SUPPORTED 0xC00000BB 100%) AND the monitor's Phase 1/2 (Session-0 caller)
# birthed INVISIBLE Session-0 SYSTEM shells + killed the visible admin shell ->
# 'it just kills cmd powershell and ise'. Fix: CreateProcessAsSystem ALWAYS pins
# the token to the active session (SetTokenInformation(TokenSessionId), no
# skip-on-equal) so CreateProcessWithTokenW births in the interactive session;
# Invoke-BornAsSystemShellVisible births + verifies the child is SYSTEM +
# Session>0 (visible) BEFORE killing the old admin PID (else leaves admin alive
# + kills the invisible failed child). Additive; sections 1-39 intact.
Add-Assertion "VisShell: Invoke-BornAsSystemShellVisible helper defined" ($gm -match 'function Invoke-BornAsSystemShellVisible') "Invoke-BornAsSystemShellVisible missing -- the visible born-as-SYSTEM fallback has no implementation"
$vbsMatch = [regex]::Match($gm, '(?s)function Invoke-BornAsSystemShellVisible \{(.*?)\nfunction Invoke-GmHookShellFeedbackElevation \{')
$vbsBody = if ($vbsMatch.Success) { $vbsMatch.Groups[1].Value } else { "" }
Add-Assertion "VisShell: Invoke-BornAsSystemShellVisible body extractable" ($vbsMatch.Success) "could not isolate Invoke-BornAsSystemShellVisible body"
if ($vbsMatch.Success) {
    Add-Assertion "VisShell: helper births via CreateProcessAsSystem (session-pinned)" ($vbsBody -match '\[TokenOps\]::CreateProcessAsSystem') "helper does not birth via CreateProcessAsSystem -- no session-pinned SYSTEM birth"
    Add-Assertion "VisShell: helper snapshots before-PIDs (beforePids) to identify the new child" ($vbsBody.Contains('$beforePids')) "helper does not snapshot before-PIDs -- cannot identify the new child (CreateProcessAsSystem discards the child PID)"
    Add-Assertion "VisShell: helper verifies the new child is SYSTEM (Test-PidIsSystem newPid)" ($vbsBody -match 'Test-PidIsSystem -ProcessId \$newPid') "helper does not verify the new child is SYSTEM -- a non-SYSTEM child could replace the admin shell"
    Add-Assertion "VisShell: helper verifies the new child is in an interactive session (newSession -gt 0, not invisible Session 0)" ($vbsBody -match 'newSession -gt 0') "helper does not check the new child's session -- an invisible Session-0 child would be kept + the visible admin shell killed ('it kills my shell')"
    Add-Assertion "VisShell: helper kills the old admin PID ONLY on verified visible SYSTEM (verified guard around Stop-Process -Id OldAdminPid)" ($vbsBody -match 'if \(\$verified\) \{[\s\S]{0,200}?Stop-Process -Id \$OldAdminPid') "helper kills the old admin PID unconditionally -- an invisible Session-0 birth would kill the visible admin shell"
    Add-Assertion "VisShell: helper kills the invisible failed child + leaves admin on NOT verified (no orphan accumulation)" ($vbsBody -match 'if \(\$newPid -gt 0\) \{[\s\S]{0,200}?Stop-Process -Id \$newPid') "helper does not kill the invisible failed child -- failed retries would orphan Session-0 SYSTEM shells"
    Add-Assertion "VisShell: helper is fail-stop (leaves admin alive + returns false on a non-visible birth)" ($vbsBody.Contains('LEFT admin') -or $vbsBody.Contains('left the admin shell alive')) "helper is not fail-stop -- it would kill the visible admin shell on a non-visible birth"
}
...
Add-Assertion "CwdPreserve: CreateProcessAsSystem passes workingDirectory + pinned envPtr through to CreateProcessWithTokenW" ($gm -match 'CreateProcessWithTokenW\(hPrimaryToken, LOGON_WITH_PROFILE, appName, cmdLine, creationFlags, envPtr, workingDirectory,') "CreateProcessAsSystem does not pass workingDirectory + the pinned envPtr to CreateProcessWithTokenW -- the new shell ignores the preserved cwd/env"
Add-Assertion "ShellEnv: TokenOps.GetProcessShellContext defined (one PEB walk -> cwd + env)" ($gm -match 'public static ShellContext GetProcessShellContext\(int pid\)') "GetProcessShellContext missing -- the born-as-SYSTEM fallback cannot recover cwd + env in one walk"
Add-Assertion "ShellEnv: ShellContext struct exposes Cwd + Environment fields (raw unicode env block)" ($gm -match 'public struct ShellContext[\s\S]{0,200}?public string Cwd;[\s\S]{0,80}?public byte\[\] Environment;') "ShellContext struct missing/incomplete -- the combined context has no env field"
Add-Assertion "ShellEnv: GetProcessShellContext reads the environment pointer at RTL_USER_PROCESS_PARAMETERS 0x80" ($gm.Contains('procParams.ToInt64() + 0x80')) "GetProcessShellContext does not read the Environment pointer at 0x80 -- the env block cannot be recovered"
Add-Assertion "ShellEnv: CreateProcessAsSystem accepts an environmentBlock param (lpEnvironment for CreateProcessWithTokenW)" ($gm -match 'public static int CreateProcessAsSystem\(int pid, string appName, string cmdLine, bool hideWindow, string workingDirectory = null, byte\[\] environmentBlock = null\)') "CreateProcessAsSystem does not accept an environmentBlock param -- the new SYSTEM shell cannot inherit the user env"
Add-Assertion "ShellEnv: CreateProcessAsSystem pins the env block (GCHandle.Alloc Pinned) + passes envPtr (AddrOfPinnedObject)" ($gm -match 'GCHandle\.Alloc\(environmentBlock, GCHandleType\.Pinned\)' -and $gm -match 'envPin\.AddrOfPinnedObject\(\)') "CreateProcessAsSystem does not pin the env block -- a GC move during CreateProcessWithTokenW would read garbage"
Add-Assertion "ShellEnv: CreateProcessAsSystem frees the env pin after the launch (GCHandle.Free)" ($gm -match 'if \(envPin\.IsAllocated\) envPin\.Free\(\)') "CreateProcessAsSystem does not free the env pin -- a pinned byte[] leaks the GCHandle table"
Add-Assertion "ShellEnv: born-shell context cache declared (\$script:BornShellContextCache) for per-OldAdminPid retry reuse" ($gm.Contains('$script:BornShellContextCache')) "the born-shell context cache is missing -- every retry tick repeats the full PEB walk for the same dying shell"
if ($vbsMatch.Success) {
    Add-Assertion "CwdPreserve: helper resolves context via GetProcessShellContext (one walk, not separate cwd+env reads)" ($vbsBody -match '\[TokenOps\]::GetProcessShellContext\(\$OldAdminPid\)') "helper does not call GetProcessShellContext -- the new SYSTEM shell cannot inherit the user's folder/env"
    Add-Assertion "CwdPreserve: helper passes the preserved cwd + env to CreateProcessAsSystem (workingDirectory + environmentBlock)" ($vbsBody -match '\[TokenOps\]::CreateProcessAsSystem\(\$SystemPid, \$ShellPath, \$cmdLine, \$false, \$preserveCwd, \$preserveEnv\)') "helper does not pass the preserved cwd + env to CreateProcessAsSystem -- the new SYSTEM shell falls back to System32 / Session-0 env"
    Add-Assertion "CwdPreserve: helper guards the context read on OldAdminPid>0 (no OpenProcess on a missing pid)" ($vbsBody -match 'if \(\$OldAdminPid -gt 0\) \{' -and $vbsBody -match '\[TokenOps\]::GetProcessShellContext\(\$OldAdminPid\)') "helper reads the context without guarding OldAdminPid>0 -- a zero pid would be passed to OpenProcess"
    Add-Assertion "CwdPreserve: helper logs whether a context was preserved (diagnostic for fail-open fallback)" ($vbsBody.Contains('Preserving old admin') -or $vbsBody.Contains('No cwd preserved') -or $vbsBody.Contains('No context preserved')) "helper does not log the context-preservation outcome -- a fail-open to System32/Session-0 env is invisible in the dump"
    Add-Assertion "ShellEnv: helper caches the resolved context per-OldAdminPid (BornShellContextCache keyed by OldAdminPid)" ($vbsBody -match '\$script:BornShellContextCache\[\$OldAdminPid\]') "helper does not cache the resolved context -- every retry repeats the PEB walk"
    Add-Assertion "ShellEnv: cache has a 5s TTL so a stale dying-shell entry is dropped (Ts -gt 5)" ($vbsBody -match '\$cached\.Ts\) -gt 5') "cache has no TTL -- a stale entry for a long-dead PID could be reused after the PID is recycled"
Add-Assertion "ShellEnv: helper clears the cache entry on verified kill (no stale context for a recycled PID)" ($vbsBody -match 'BornShellContextCache\.Remove\(\$OldAdminPid\)') "helper does not clear the cache on verified kill -- a recycled PID could inherit a stale cwd/env"
}

# Part G -- gmhook direct Session-1 SYSTEM shell birth (2026-07-24). The VM dump
# proved the monitor (Session 0) CANNOT birth a visible SYSTEM shell on Win11
# 26100: every born-as-SYSTEM child lands session=0 (invisible) + is killed,
# while the admin shell stays admin forever. Only a Session-1 process can birth
# a Session-1 child. gmhook is injected into explorer.exe (Session 1) and already
# holds a stolen SYSTEM token + CreateProcessWithTokenW, so it births the
# interactive shell as SYSTEM IN SESSION 1 directly (visible), with the host's
# cwd + env, falling back to a normal user birth + SignalShellBirth if the
# SYSTEM birth fails. The monitor's Session-0 birth is now a last-resort path
# guarded by a Session-0-blocked skip so it does not spawn + kill invisible
# shells every retry tick. Additive.
Add-Assertion "GmHookDirect: interactive shells are BORN AS SYSTEM first via TryCreateProcessWithSystemToken (IsInteractiveShell branch in the hook)" ($gmhook -match 'if \(IsInteractiveShell\(baseName\)\) \{[\s\S]{0,1200}?TryCreateProcessWithSystemToken\(pCpwt, lpApplicationName, lpCommandLine,') "gmhook does not birth interactive shells as SYSTEM first -- the Session-0 monitor birth is the only (invisible) path"
Add-Assertion "GmHookDirect: SYSTEM birth falls back to the real CreateProcessW + SignalShellBirth on failure (shell never vanishes)" ($gmhook -match 'if \(IsInteractiveShell\(baseName\)\) \{[\s\S]{0,3000}?pOrigCreateProcessW\(lpApplicationName, lpCommandLine,[\s\S]{0,400}?SignalShellBirth\(lpProcessInformation->dwProcessId\)') "gmhook does not fall back to a normal user birth + signal -- a failed SYSTEM birth would vanish the shell"
Add-Assertion "GmHookDirect: env parity via GetEnvironmentStringsW when lpEnvironment is NULL (SYSTEM shell inherits the user env, not SYSTEM's)" ($gmhook.Contains('GetEnvironmentStringsW') -and $gmhook.Contains('CREATE_UNICODE_ENVIRONMENT') -and $gmhook.Contains('FreeEnvironmentStringsW')) "gmhook does not pass the host/user env to the SYSTEM shell -- it would get SYSTEM's System32 profile instead of %USERPROFILE%"
Add-Assertion "GmHookDirect: critical OS processes still passthrough untouched (never born as SYSTEM)" ($gmhook -match 'if \(IsCriticalProcess\(baseName\)\) \{[\s\S]{0,400}?pOrigCreateProcessW\(lpApplicationName, lpCommandLine,[\s\S]{0,200}?InterlockedExchange\(&inHook, 0\);\s*\n\s*return result;') "gmhook touches critical OS processes -- csrss/lsass/winlogon could be destabilized"
Add-Assertion "GmHookDirect: other launcher hosts (wt/conhost/explorer) still passthrough (not born as SYSTEM)" ($gmhook -match 'if \(IsShellLauncherProcess\(baseName\)\) \{[\s\S]{0,400}?pOrigCreateProcessW\(lpApplicationName, lpCommandLine,[\s\S]{0,200}?InterlockedExchange\(&inHook, 0\);\s*\n\s*return result;') "gmhook births launcher hosts as SYSTEM -- the desktop/taskbar/console association would break"
Add-Assertion "GmHookDirect: the old single passthrough (IsCriticalProcess || IsShellLauncherProcess) is GONE (shells no longer lumped with launchers)" (-not ($gmhook -match 'if \(IsCriticalProcess\(baseName\) \|\| IsShellLauncherProcess\(baseName\)\)')) "gmhook still lumps critical + launcher + shell into one passthrough -- interactive shells are not birthed as SYSTEM"
Add-Assertion "Sess0Block: monitor Session-0-blocked set declared (\$script:Session0BlockedPids)" ($gm.Contains('$script:Session0BlockedPids')) "the Session-0-blocked set is missing -- the monitor re-births invisible Session-0 shells every retry tick"
Add-Assertion "Sess0Block: helper short-circuits a Session-0-blocked OldAdminPid (leaves admin alive, no re-birth)" ($vbsBody -match 'if \(\$script:Session0BlockedPids\.ContainsKey\(\$OldAdminPid\)\)[\s\S]{0,400}?leaving admin alive, no re-birth') "helper does not short-circuit a Session-0-blocked PID -- it keeps spawning invisible Session-0 shells every tick"
