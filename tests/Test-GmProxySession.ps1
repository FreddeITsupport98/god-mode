...
Add-Assertion "God-Mode-Windows.ps1: candidates filter drops BROWSERS only (no AppX drop in filter -- UWP stay hooked)" ($gm.Contains('CompatExclusions.Browser.ContainsKey') -and -not $gm.Contains('CompatExclusions.AppX.ContainsKey($baseKey)')) "candidates filter still drops AppX -- UWP/AppX must stay hooked (try SYSTEM first, Detector B catches refusals)"
...
Add-Assertion "God-Mode-Windows.ps1: seed filtered browser-only (filteredSeed + droppedBrowserNames, NO seedDroppedAppx)" ($gm.Contains('filteredSeed') -and $gm.Contains('droppedBrowserNames') -and -not $gm.Contains('seedDroppedAppx')) "seed compat filtering missing or still drops AppX (UWP must stay hooked)"
...
Add-Assertion "God-Mode-Windows.ps1: candidate filter drops BROWSERS only (no AppX drop -- UWP stay hooked)" ($gm.Contains('CompatExclusions.Browser.ContainsKey') -and -not $gm.Contains('CompatExclusions.AppX.ContainsKey($baseKey)')) "candidate filter still drops AppX -- UWP/AppX must stay hooked (try SYSTEM first)"
