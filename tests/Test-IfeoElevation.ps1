...
Add-Assertion "Detector A: candidates filter drops BROWSERS only (CompatExclusions.Browser.ContainsKey, NO AppX drop)" ($candBody.Contains('CompatExclusions.Browser.ContainsKey') -and -not $candBody.Contains('CompatExclusions.AppX.ContainsKey')) "candidates filter still drops AppX -- UWP/AppX must stay hooked (try SYSTEM first, Detector B catches refusals)"
...
Add-Assertion "Detector A: Install-IfeoElevation counts browser drops (seedDroppedBrowser, NO seedDroppedAppx)" ($installBody.Contains('seedDroppedBrowser') -and -not $installBody.Contains('seedDroppedAppx')) "Install-IfeoElevation still counts AppX drops -- UWP must stay hooked (browser-only drop)"
...
Add-Assertion "Detector A: IFEO log line reflects browser-only strategy (droppedBrowser, UWP stay hooked)" ($installBody.Contains('droppedBrowser=$seedDroppedBrowser') -and $installBody.Contains('UWP/AppX stay hooked')) "IFEO log line does not reflect the browser-only strategy"
